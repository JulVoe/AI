...
struct AugmentationInfo2D_OUT {
   DATA_FORMAT format;          //Transform data to this format
    ...
    bool do_input_aug;           //If true, performs rotate, flip and crop from input augmentation also on output (useful for segmentatin but not classification)
...
private:
    ...
    */
    void worker_thread(uint64_t id) {
        ...
        avx_xorshift128plus_init(r ^ (r << 5) ^ (r >> 19), r ^ (r << 39) ^ (r >> 27), my_key);  //TODO:FIXIT

        //2.: Set AVX constants (TODO/FIXIT: WRONG CONVERSION)
        ...
        while (thread_status.load() < WORKER_STATUS::STOP) {                                      //Break if exit signal is reached
            ...
            for (uint32_t b = 0; b != augmentation_batches_cpu; b++) {                            //Go through all batches in augmentation tile
                ...
                if (expected == BATCH_STATUS::USED) {                                             //Since expected was not changed, batch was used and is now getting reloaded
                    ...
                    T* aug_in  = &aug_tiles[0][b * (batch_size * sample_size[2])];
                    T* aug_out = &aug_tiles[1][b * (batch_size * sample_size[3])];
                    ...
                    else {                                                                        //Input source did change. expected is now set to the new batch status, as we will reload the batch again and this is now the status befor we begin
                        //As the input source changed, this is now the new input source
                        training = (training == WORKER_STATUS::TRAINING) ? WORKER_STATUS::VALIDATION : WORKER_STATUS::TRAINING;
                ...
                if(ready_gpu[ind].load() == BATCH_STATUS::USED) {
                    ...
                    for(uint32_t cpu_ind = 0; cpu_ind != augmentation_batches_cpu; cpu_ind++){
                        ...
                        if(expected == BATCH_STATUS::READY){                                           //Cpu tile was READY
                            ...
                            cudaMemcpyAsync(in , &aug_tiles[0][cpu_ind * (batch_size * sample_size[2])], batch_size * sample_size[2] * sizeof(T), cudaMemcpyHostToDevice, *cur_stream);
                            cudaMemcpyAsync(out, &aug_tiles[1][cpu_ind * (batch_size * sample_size[3])], batch_size * sample_size[3] * sizeof(T), cudaMemcpyHostToDevice, *cur_stream);
                            cudaEventRecord(gpu_sync[ind], *cur_stream);
...
#define PSIZE 6
    ...
    */
    void debug_thread() {
        ...
        //3.: Key handler
        glfwSetWindowUserPointer(window, (void*)workers_tile);                //Key handler needs to acces this, as it resets these values when "R" is pressed
        ...
        while (thread_status.load() < WORKER_STATUS::STOP && !glfwWindowShouldClose(window)) {
            ...
            for (uint32_t ind = 0; ind != augmentation_batches_cpu * batch_size; ind++) {
                ...
                shrinkToGL<T>(&aug_tiles[0][ind * sample_size[2]], sample_shape[2], (uint8_t*)buffer);
                    //Image::show<uint8_t, true>((uint8_t*)buffer, in_shape, Image::CHANNEL_ORDER::CHANNELS_LAST);
                Image::resize<uint8_t, Image::CHANNEL_ORDER::CHANNELS_LAST>((uint8_t*)buffer, sample_shape[2], Offset2D<uint32_t>(XIMG, YIMG));
                ...
                if (ind % batch_size == 0) {
                    ...
                    int32_t val = ready_cpu[ind / batch_size];
                    glBegin(GL_QUADS);
            ...
            for (uint32_t ind = 0; ind != augmentation_batches_gpu * batch_size; ind++) {
                ...
                cudaStreamSynchronize((cudaStream_t)0);
                    ...
                    //Image::show<T, false>(buffer, in_shape);
                ...
                if (ind % batch_size == 0) {
                    ...
                    if (val >= 0) {
                        glBegin(GL_LINES);
                        glColor3f(1.0, 1.0, 1.0);
                        glVertex2f(val * (XIMG + PADDING) + PADDING + XIMG / 2, y - PADDING - GAP);
...
public:
    ...
    */
    DatasetHandler(char* in, char* out, float train_split, uint32_t mb_cpu) :
        DatasetHandler(fopen(in, "r"), fopen(out, "r"), train_split, mb_cpu)
    {}
    ...
    */
    inline void start_workers(uint32_t n, uint32_t num_streams, WORKER_STATUS stat = WORKER_STATUS::IDLE) {
        ...
        for (uint32_t u = 0; u != num_workers; u++) {
            new (&workers[u]) std::thread(&DatasetHandler::worker_thread, this, u);    //TODO: LLDB bug permits
    ...
    */
    template<bool training> void setDataInput() {
        ...
        for (uint32_t u = 0; u != augmentation_batches_cpu; u++) {
            ...
            BATCH_STATUS stored_loading = (BATCH_STATUS)((int32_t)stored_value & ~(1u << 31));    //stored_value but highest bit is not set. When stored value was positiv (exactly when LOADING status), it stayes the same. Otherwise, it changes
            BATCH_STATUS toggled_deprecated = (BATCH_STATUS)((int32_t)stored_value ^ (int32_t)BATCH_STATUS::DEPRECATED); //stored_value with flipped DEPRECATED bit
    ...
    template<bool training> void advance(T*& in, T*& out) {
        ...
        //4.: Synchronize
        while(ready_gpu[(in - gpu_tiles[0]) / (batch_size * sample_size[2])].load() != BATCH_STATUS::READY) {} //Wait until data is ready
    }
...
int main() {
    ...
    handler.set_batching(8, 2, 2);
    handler.start_workers(8, 8, WORKER_STATUS::TRAINING);
